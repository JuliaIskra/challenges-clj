...
(defn greatest-product-of-n-adjacent-numbers
  [n numbers]
  (loop [start-index 0
         max-product 0]
    (if (>= start-index (- (count numbers) (- n 1)))
      max-product
      (let [digits  (subvec numbers start-index (+ start-index n))
            product (reduce * digits)]
        (recur (inc start-index) (if (< max-product product) product max-product))))))
...
(defn problem8
  ...
  []
  (let [numbers (vec (map #(Integer/parseInt %1)
                          (map str (str "73167176531330624919225119674426574742355349194934"
                                        ...
                                        "05886116467109405077541002256983155200055935729725"
                                        "71636269561882670428252483600823257530420752963450"))))]
    (greatest-product-of-n-adjacent-numbers 13 numbers)))
...
(defn problem11
  ...
  (let [numbers           (->> ["08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08"
        ...
        diag-lurd-product (map #(greatest-product-of-n-adjacent-numbers 4 (vec %1)) diag-lurd)
        diag-ruld-product (map #(greatest-product-of-n-adjacent-numbers 4 (vec %1)) diag-ruld)]
    (max
